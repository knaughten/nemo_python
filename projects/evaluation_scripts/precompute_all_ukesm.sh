...
IN_DIR=/gws/ssde/j25b/terrafirma/tarlge/ukesm_monitoring/data/u-${SUITE}

# Timeseries and Hovmollers - update existing files, or calculate from scratch
python -c "from nemo_python.projects.evaluation import *; update_timeseries_evaluation_UKESM1('"${SUITE}"', in_dir='"${IN_DIR}"', out_dir='"${SUITE}"')"
python -c "from nemo_python.projects.evaluation import *; update_hovmollers_evaluation_UKESM1('"${SUITE}"', in_dir='"${IN_DIR}"', out_dir='"${SUITE}"')"
