...
python -c "from nemo_python.projects.evaluation import *; update_timeseries_evaluation_UKESM('"${SUITE}"', in_dir='./', version=2)"
...
for OPTION in "${OPTIONS[@]}"; do
    ...
    python -c "from nemo_python.projects.evaluation import *; precompute_avg(option='"${OPTION}"', config='UKESM2', suite_id='"${SUITE}"', in_dir='./')"
done
