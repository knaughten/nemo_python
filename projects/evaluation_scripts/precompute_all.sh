...
for f in bottom_TS_avg.nc zonal_TS_avg.nc seaice_avg.nc; do
    if [ -f $f ]; then
	rm $f
    fi
done
...
python -c "from nemo_python.projects.evaluation import *; precompute_avg(option='seaice', out_file='seaice_avg.nc')"
