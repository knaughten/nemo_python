...
suites_ramp_down_rates = {'8 Gt/y' : ['di335', 'da800', 'da697', 'da892', 'df453', 'dc251', 'de943', 'dg093', 'dm357'],
                          '4 Gt/y' : ['dc051', 'dc052', 'dc248', 'dc249', 'dc565', 'dd210', 'dc032', 'df028', 'do135', 'dc130', 'de962', 'dg094', 'dm358'],
                          '2 Gt/y' : ['df025', 'df027', 'df021', 'do136', 'dh541', 'dh859', 'de963', 'dg095', 'dm359']}
# Dictionary of overshoot lengths
suites_overshoot_lengths = {'50 years': ['da697', 'dc052', 'di335', 'dc051', 'df025', 'df453', 'df028', 'df021', 'da892', 'do135', 'dh541', 'dc251', 'dc130', 'de943', 'de962', 'de963'],
                            '200 years': ['dc248', 'da800', 'dc565', 'df027', 'dc249', 'do136', 'dh859', 'dd210', 'dg093', 'dg094', 'dg095', 'dm357', 'dm358', 'dm359'],
                            '30 years': ['dc032']}
...
def plot_FW_timeseries (base_dir='./'):
    ...
    pi_years = 50
    var_names = ['all_massloss', 'ross_massloss', 'all_seaice_meltfreeze', 'all_pminuse', 'all_runoff', 'all_iceberg_melt']
    var_titles = ['Ice shelves', 'Ross Ice Shelf', 'Sea ice', 'Precip - evap', 'Surface melt',  'Icebergs']
    timeseries_files = 2*['timeseries.nc'] + 4*['timeseries_sfc.nc']
    factors = 2*[rho_fw/rho_ice*1e6/sec_per_year] + 4*[1e-3/sec_per_year]
    glacial = [True, True, False, False, True, True]  # Ross is glacial of course, but don't want to double-count
    units = 'mSv'
    colours = [(0.6,0.6,0.6), 'black', (0.8,0.47,0.65), (0,0.62,0.45), (0,0.45,0.7), (0.9,0.62,0)]   
    ...
    label_y = 120
    percent_titles = ['% total anomaly from glacial sources', '% total anomaly from Ross Ice Shelf']
    threshold = 5  # Do not show percent contributions when total FW flux is < 5 mSv
    percent_colours = ['CornflowerBlue', 'RebeccaPurple']
    ...
    # Sum fields for bottom panel
    # Total glacial sources, and total FW anomalies from all sources
    data_glacial = 0*data_plot[0]
    data_total = 0*data_plot[0]
    for v in range(num_vars):
        if var_names[v] == 'ross_massloss':
            # Skip so don't double count
            v_ross = v
            continue
        data = data_plot[v]
        if glacial[v]:
    ...
    percent_glacial = (data_glacial/data_total*1e2).where(data_total > threshold)
    percent_ross = (data_plot[v_ross]/data_total*1e2).where(data_total > threshold)
    plot_percent = [percent_glacial, percent_ross]
    print('Max Ross contribution to total is '+str(percent_ross.max().item())+'%')
    ...
    years = time_in_years(data_plot[0], year0=year0)

    # Plot
    fig = plt.figure(figsize=(8,5.5))
    gs = plt.GridSpec(5,1)
    gs.update(left=0.08, right=0.99, bottom=0.1, top=0.93, hspace=0.5)
    # Top panel
    ax1 = plt.subplot(gs[:3,0])
    for v in range(len(data_plot)):
        ax1.plot(years, data_plot[v], '-', color=colours[v], label=var_titles[v], linewidth=1)
    # Bottom panel
    ax2 = plt.subplot(gs[3:,0])
    for v in range(len(plot_percent)):
        ax2.plot(years, plot_percent[v], '-', color=percent_colours[v], label=percent_titles[v], linewidth=1)
    # Shade each stage
    ...
    for r in range(len(tip_regions)):
        if tip_times[r] is not None:
            ...
            ax1.text(tip_year, label_y, tip_titles[r]+' tips', ha='left', va='top', rotation=-90)
            if recover_times[r] is not None:
                ...
                ax1.text(recover_year, label_y, tip_titles[r]+' recovers', ha='left', va='top', rotation=-90)
    ax2.set_ylim([-25, 150])
    for ax, ypos in zip([ax1, ax2], [7, 11]):
        ax.grid(linestyle='dotted')
        ...
        for suite_string, shelf_bwsalt, cavity_temp, massloss in zip(suite_strings, all_shelf_bwsalt, all_cavity_temp, all_massloss):
            ...
            if tips:
                ...
                if ramp_down_time is not None and tip_time > ramp_down_time:
                    # Also includes restabilisation case
                    tip_ramp_down += 1
...
def map_snapshots (var_name='bwtemp', base_dir='./'):
    ...
    mask_pad = 5e4
    ice_dir = base_dir #'/gws/ssde/j25b/terrafirma/tm17544/TerraFIRMA_overshoots/raw_data/'
    ice_file_head = '/bisicles_' #'/icesheet/bisicles_'
    ice_file_mid = 'c_'
...
def plot_particle_tracking (base_dir='./'):
    ...
    lon_2d, lat_2d = np.meshgrid(bins_lon, bins_lat)
    ...
    print('Selecting particles released in first '+str(year_max-year_tip)+' years after tipping')
    ...
    for t in range(len(age_bounds)):
        print('Selecting age group '+str(t+1))
        ...
        # Error checking: make sure they have NaNs at the same places
        ...
        # Get probability distribution of particles using 2D histogram onto regular grid
        ...
        hist_plot.append(hist)        

    # Plot
    fig = plt.figure(figsize=(7,8))
    gs = plt.GridSpec(2,2)
    gs.update(left=0.05, right=0.95, bottom=0.12, top=0.9, hspace=0.15, wspace=0.1)
    cax = fig.add_axes([0.3, 0.07, 0.4, 0.03])
    for t in range(len(age_bounds)):
        ax = plt.subplot(gs[t//2,t%2])
        ax.axis('equal')
        # Empty plot showing grounded ice (grey), ocean (white), ice fronts (black)
        ...
        ax.set_xticks([])
        ax.set_yticks([])
    plt.suptitle('Distribution of particles released with Ross meltwater', fontsize=16)
